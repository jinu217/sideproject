...
// ----------------------
// 2) 프로젝트 참여 인원 조회 (users + user_profiles)
// ----------------------
$sql = "
    SELECT 
        u.id   AS user_id,
            ...
            <?php foreach ($memberRows as $row): ?>
                <div class="frame-group">
                    <?php foreach ($row as $m): ?>
                        <?php
                            // 03_view.php처럼 프로필 이미지 결정
                            $profileImage = !empty($m['profile_image_url'])
                                ? $m['profile_image_url']
                                : '../img/default_profile.png';
                        ?>
                        <div class="frame-wrapper2">
                            <div
                                class="frame-div member-card"
...
<script>
  ...
  document.querySelectorAll('.member-card').forEach(function(card) {
      card.addEventListener('click', function () {
          ...
          const url =
              "07_03_review.php"   // 📌 파일명 07_03_view.php 기준
